...
if (empty($current_category_id)){
				...
				$best_sellers_query = tep_db_query("SELECT manufacturers.manufacturers_name, manufacturers.manufacturers_id FROM `statique_best_seller` INNER JOIN manufacturers ON statique_best_seller.marque = manufacturers.manufacturers_name where type='marque' LIMIT 0 , 30");
									

				echo '<td class="box_body box_body_td"><ul>';								
				while($products = tep_db_fetch_array( $best_sellers_query)) 
   					 {
					 echo '<li class="box_body bg_list"><a  href="index.php?manufacturers_id='.$products['manufacturers_id'].'">'.$products['manufacturers_name'].'</a>';
...
while($products = tep_db_fetch_array( $best_sellers_query)) 
   					 {
					 echo '<a  href="index.php?manufacturers_id='.$products['products_id'].'">'.$products['products_name'].'</a>';
...
WHERE products_to_categories.categories_id ='. $current_category_id .' and manufacturers.manufacturers_name like \''.$lettre.'%\' ORDER BY manufacturers_name';
...
echo '<table border="0" width="100%" cellspacing="0" cellpadding="0"  class="box_heading_table_2">
  <tr>
    <td><img src="images/box_corn_l_2.gif" border="0" alt=""></td>
    <td  style="width:100%;" class="box_heading_td_2">'.$titre.'</td>
    <td><img src="images/box_corn_r_2.gif" border="0" alt=""></td>
  </tr>
</table>
';	
