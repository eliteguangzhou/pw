  ...
  function tep_get_countries($countries_id = '', $with_iso_codes = false) {
    ...
    if (tep_not_null($countries_id)) {
      if ($with_iso_codes == true) {
        $countries = tep_db_query("select countries_name, countries_iso_code_2, countries_iso_code_3 from " . TABLE_COUNTRIES . " where enabled = 1 and countries_id = '" . (int)$countries_id . "' order by countries_name");
        $countries_values = tep_db_fetch_array($countries);
        $countries_array = array('countries_name' => $countries_values['countries_name'],
                                 'countries_iso_code_2' => $countries_values['countries_iso_code_2'],
                                 'countries_iso_code_3' => $countries_values['countries_iso_code_3']);
      } else {
        $countries = tep_db_query("select countries_name from " . TABLE_COUNTRIES . " where enabled = 1 and countries_id = '" . (int)$countries_id . "'");
        $countries_values = tep_db_fetch_array($countries);
        $countries_array = array('countries_name' => $countries_values['countries_name']);
      }
    } else {
      $countries = tep_db_query("select countries_id, countries_name from " . TABLE_COUNTRIES . " where enabled = 1 order by countries_name");
...
//                    e.g. info@mytepshop.com
  ...
  function tep_mail($to_name, $to_email_address, $email_subject, $email_text, $from_email_name, $from_email_address) {
    ...
    //Traitement requete car le serveur smtp n'envoie pas si le from est un domaine different du sien
    if ($from_email_address != STORE_OWNER_EMAIL_ADDRESS) {
        $email_text = "Requete de " .$from_email_name." < " . $from_email_address . ">\n" . $email_text;
    ...
    $mail->Username = STORE_OWNER_EMAIL_ADDRESS; // GMail username (including @gmail.com)
...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  ...
  function display_product_name($name, $product, $display_model_id = false) {
    ...
    if(count($patterns[0]) < 2)
        $desc = get_desc($product['products_description']) . '<br />' .strip_tags(get_ml($patterns[0][0]));
    else
