  ...
  if (!tep_session_is_registered('language') || isset($HTTP_GET_VARS['language'])) {
    ...
    if (isset($HTTP_GET_VARS['language']) && tep_not_null($HTTP_GET_VARS['language'])) {
      ...
      //$lng->get_browser_language();
        //Temp added by clément to fix lang in french
        $lng->set_language('fr');
    }
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'update_product' : if (empty($_POST['coupon_code'])) {
                                for ($i=0, $n=sizeof($HTTP_POST_VARS['products_id']); $i<$n; $i++) {
                                  ...
                                  tep_redirect(tep_href_link($goto . '?recalculate', tep_get_all_get_params($parameters)));
                              }
                              break;
...
include (DIR_WS_CLASSES.'card_member.php');
$card_member = new card_member(!empty($customer_email_address) ? $customer_email_address : false);
?>